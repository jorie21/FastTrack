...
<template>
    <div class="flex-1 max-w-xl animate-fade-in-up">
        ...
        <h1 class="mb-6 font-extrabold leading-[1.05] tracking-[-2.5px]"
            style="font-size: clamp(44px, 5.5vw, 72px);"
        >
            Track
        ...
        <!-- Subheadline -->
        <p class="mb-10 text-[17px] leading-relaxed text-white/55 max-w-md" style="animation-delay: 0.1s;">
            FastTrack gives you a crystal-clear picture of your cash flow —
            where it comes from, where it goes, and how fast it moves.
        </p>

        <!-- CTA buttons -->
        <div class="flex items-center gap-3.5 mb-12" style="animation-delay: 0.2s;">
        ...
        <!-- Stats row -->
        <div class="flex items-center gap-6 pt-5 border-t border-white/8" style="animation-delay: 0.3s;">
            <template v-for="(stat, i) in stats" :key="i">
                ...
                <div v-if="i < stats.length - 1" class="w-px h-9 bg-white/10" />
            </template>
        </div>
