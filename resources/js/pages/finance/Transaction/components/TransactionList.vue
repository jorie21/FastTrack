...
<template>
    ...
    <div v-if="transactions.length > 0" class="flex flex-col gap-2.5">
        <div
            ...
            :key="txn.id"
            class="group flex items-center gap-4 rounded-2xl border border-white/6 px-5 py-4 transition-all duration-200 hover:border-white/12"
        ...
        >
            ...
            >
                {{ categories.find(c => c.name === txn.category)?.icon ?? '💳' }}
            </div>

            <!-- Info -->
            <div class="min-w-0 flex-1">
                <p class="truncate text-sm font-semibold text-white/90">{{ txn.title }}</p>
                <div class="mt-0.5 flex items-center gap-2">
                    <span
                        class="rounded-full px-2 py-0.5 font-mono text-[10px]"
                        :style="{ color: txn.categoryColor, background: txn.categoryColor + '18' }"
                    >{{ txn.category }}</span>
    ...
    <div v-else class="flex flex-col items-center justify-center gap-3 py-16">
        <div class="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/8 bg-white/5 text-2xl">🔍</div>
